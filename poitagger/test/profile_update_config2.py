...
C["PATHS"]["rootdir"] = "C:/Testbverzeichnis"
C['IMPORT']['nurFlugBilder'] =  'traaaaue'
...
for i in dict_generator(C):
    if i[2] == None: 
        update = "UPDATE Config SET value = ? WHERE name = '"+i[1]+"' AND group_fk = '"+str(CG[i[0]])+"' "
        db.cur.execute(update,(None,))
    else:
